#!/usr/bin/env python3
"""Build static Iliad reader data from the pinned Perseus TEI source."""
...
REFERENCE_SECTIONS = {
    "glossary": {"file": "glossary.json", "minimum": 1, "titleField": "nameKo"},
    "people": {"file": "people.json", "minimum": 1, "titleField": "nameKo"},
    "places": {"file": "places.json", "minimum": 1, "titleField": "nameKo"},
    "textualNotes": {
...
EXPECTED_BOOKS = 24
EXPECTED_LINES = 15687
EXPECTED_PARAGRAPHS = 1054
EXPECTED_PARA_MARKERS = 1047
EXPECTED_GAPS = {
    "9": [458, 459, 460, 461],
    "11": [543],
    "14": [269],
...
def parse_source(source_path):
    ...
    for book_node in book_nodes:
        ...
        for paragraph_index, lines in enumerate(paragraphs, start=1):
            paragraph_id = "il-{:02d}-p{:03d}".format(book_number, paragraph_index)
