...
export function IliadReader({
  ...
  useEffect(() => {
    ...
    const savedScale = Number(
      window.localStorage.getItem('iliad-reader-scale'),
    );
    const savedTheme = window.localStorage.getItem('iliad-theme');
    ...
    const stored = window.localStorage.getItem('iliad-reading-position');
    if (stored) {
      try {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('iliad-theme', dark ? 'dark' : 'light');
  }, [dark]);
  ...
  const changeViewMode = (mode: ViewMode) => {
    setViewMode(mode);
    window.localStorage.setItem('iliad-view-mode', mode);
  };
  ...
  const changeScale = (delta: number) => {
    ...
    setReaderScale(next);
    window.localStorage.setItem('iliad-reader-scale', String(next));
  };

  const copyParagraph = async (paragraph: Paragraph) => {
    const parts = [
      `『일리아스』 제${paragraph.book}권 ${paragraph.lineStart}–${paragraph.lineEnd}행`,
      '',
      paragraph.greek,
    ];
    ...
    parts.push(
      '',
      `https://superantichrist.github.io/iliad-reader/?book=${paragraph.book}&line=${paragraph.lineStart}`,
    );
